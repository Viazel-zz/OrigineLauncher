...
document.getElementById('refreshMediaButton').onclick = (e) => {
    ...
    DistroManager.pullRemote().then((data) => {
        ...
        setOverlayContent(
            'Launcher Rafraichi !',
            'Ceci est une confirmation vous informant que vous avez actualisé manuellement votre lanceur, votre liste de serveurs est maintenant à jour et devrait être prête à partir ! Si vous avez des problèmes s\'il vous plaît laissez-nous savoir!',
            'Super! Merci.',
            ...
            setDismissHandler(() => {
                shell.openExternal('https://discord.gg/origine')
            })
            toggleOverlay(true, true)
        }).catch(err => {
            setOverlayContent(
                'Erreur lors de l\'actualisation de la distribution',
                'Nous n\'avons pas pu récupérer les dernières informations du serveur sur Internet au démarrage, nous avons donc utilisé une version précédemment stockée à la place.<br><br>Ce n\'est pas recommandé, et vous devez redémarrer votre client pour résoudre ce problème afin d\'éviter vos fichiers modpack. être obsolète. Si vous souhaitez continuer à utiliser le lanceur, vous pouvez réessayer à tout moment en appuyant sur le bouton d\'actualisation sur l\'écran d\'accueil.<br><br>Si cela continue de se produire et que vous ne savez pas trop pourquoi, venez nous voir sur Discord !<br><br>Code d\'erreur :<br>' + err,
                'Entendu.',
                'Rejoins notre discord'
        )
        setOverlayHandler(() => {
    ...
    setTimeout(() => {
        if(hasRPC){
            if(serv){
                ...
                DiscordWrapper.updateDetails('Ecran principal...')
            }
        }
    }, 1000)
}
        ...
        if(m.context === 'validateJava'){
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            ...
                            'Une installation x64 valide de Java 8 est requise pour le lancement.<br><br>Veuillez vous référer à notre <a href="https://github.com/VicariousNetwork/VNLauncher/wiki/Java-Management#manually-installing-a -valid-version-of-java">Guide de gestion Java</a> pour obtenir des instructions sur l\'installation manuelle de Java.',
                            'Je comprends',
                            'Retourner'
                        )
                        setOverlayHandler(() => {
            ...
            if(m.result === true){
                ...
                setOverlayContent(
                    ...
                    'Malheureusement, nous avons rencontré un problème lors de la tentative d\'installation de Java. Vous devrez installer manuellement une copie. Veuillez consulter notre <a href="https://github.com/dscalzi/VNLauncher/wiki">Guide de dépannage</a> pour plus de détails et d\'instructions.',
                    'Je comprends'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Explore la Realm!')
                    } else if(GAME_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Navigation vers le réseau Origine!')
                        DiscordWrapper.resetTime()
                    }
                }
                ...
                const gameCrashReportListener = function(data){
                    ...
                    if(data.includes('---- Minecraft Crash Report ----')){
                        ...
                        setOverlayContent(
                            'Le jeu a crash !',
                            'Euh oh ! Il semble que votre jeu vient de planter. Nous avons ouvert le dossier des rapports de plantage afin que vous puissiez facilement le partager avec notre équipe sur Discord. Si vous avez des plantages récurrents, nous vous recommandons toujours de venir nous voir !<br><br>Pour référence future, votre fichier de rapport de plantage est : <br>' + CRASH_REPORT_NAME,
                            'Ok, merci !',
                            'Ouvrir le rapport d incident'
                        )
                        setOverlayHandler(() => {
                            toggleOverlay(false)
                        })
...
function validateServerInformation() {
    ...
    DiscordWrapper.updateDetails('Chargement des informations serveur...')
                ...
                if(!ConfigManager.getServerCodes().includes(selectedServ.getServerCode())){
                    if(errorOverlay){
                        setOverlayContent(
                            ...
                            'Il semble que vous n\'ayez plus le code Modpack nécessaire pour accéder à ce serveur ! Veuillez changer de serveur pour jouer.<br><br>Si vous pensez qu\'il s\'agit d\'une erreur, veuillez contacter l\'administrateur du serveur',
                            'Changer de modpack'
                        )
                        setOverlayHandler(() => {
                            toggleServerSelection(true)
...
document.getElementById('newsButton').onclick = () => {
    ...
    if(newsActive){
        ...
        if(hasRPC){
            DiscordWrapper.updateDetails('Lit les news...')
            DiscordWrapper.clearState()
        }
    }
    slide_(!newsActive)
