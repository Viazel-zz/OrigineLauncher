...
ipcRenderer.on('MSALoginWindowReply', (_, ...arguments_) => {
    if (arguments_[0] === 'error') {
        ...
        switch (arguments_[1]) {
            case 'AlreadyOpenException': {
                setOverlayContent('ERREUR', 'Il y a déjà une fenêtre de connexion ouverte !', 'OK')
                setOverlayHandler(() => {
                    toggleOverlay(false)
            ...
            case 'AuthNotFinished': {
                setOverlayContent('ERREUR', 'Vous devez terminer le processus de connexion pour utiliser Helios Launcher. La fenêtre se fermera d\'elle-même lorsque vous vous serez connecté avec succès.', 'OK')
                setOverlayHandler(() => {
                    toggleOverlay(false)
                    toggleOverlay(false, false, 'msOverlay')
                })
    ...
    AuthManager.addMSAccount(authCode).then(account => {
        ...
        loginMSButton.disabled = false
        loginLoading(false)
        setOverlayContent('ERREUR', error.message ? error.message : 'Une erreur s\'est produite lors de la connexion avec Microsoft ! Pour plus d\'informations, veuillez consulter le journal. Vous pouvez l\'ouvrir avec CTRL + SHIFT + I.', Lang.queryJS('login.tryAgain'))
        setOverlayHandler(() => {
            formDisabled(false)
            toggleOverlay(false)
        })
