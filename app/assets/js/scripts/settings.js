...
document.getElementById('settingsAddAccount').onclick = (e) => {
    switchView(getCurrentView(), VIEWS.login, 250, 250, () => {
        ...
        if(hasRPC){
            DiscordWrapper.updateDetails('Ajouter un compte...')
...
function bindAuthAccountSelect() {
    Array.from(document.getElementsByClassName('settingsAuthAccountSelect')).map((val) => {
        val.onclick = (e) => {
            ...
            val.innerHTML = 'Compte sélectioner &#10004;'
...
function refreshAuthAccountSelected(uuid) {
    Array.from(document.getElementsByClassName('settingsAuthAccount')).map((val) => {
        ...
        if (uuid === val.getAttribute('uuid')) {
            ...
            selBtn.innerHTML = 'Compte sélectioner &#10004;'
        } else {
        ...
        authAccountStr += `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Compte sélectioner &#10004;' : '>Select Account'}</button>
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = () => {
            ...
            if (res) {
                ...
                setOverlayContent(
                    `Échec de la suppression<br>Module d'insertion ${fullName}`,
        ...
        if (dropinUI != null) {
            ...
            if (DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled) {
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if (!isOverlayVisible()) {
                        setOverlayContent(
                            'Échec de la bascule<br>un ou plusieurs mods',
                            err.message,
                            'Ok'
                        )
                        setOverlayHandler(null)
...
function populateReleaseNotes() {
    $.ajax({
        url: 'https://github.com/Viazel/OrigineLauncher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data) {
    if (data != null) {
        ...
        settingsUpdateTitle.innerHTML = 'Vous êtes entrain de lancer la dernière version'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Vérifier les mises à jours', false, () => {
            if (!isDev) {
